        ...
        <ul>
          <li v-for="game in games" :key="game.log_file">
            <button @click="selectGame(game)">
              游戏 #{{ game.game_num }} — 回合: {{ game.rounds }} — 胜者: {{ game.winner }} — {{ formatTime(game.timestamp) }}
            </button>
          </li>
        </ul>
      </div>
...
<style scoped>
.training-viewer { display:flex; gap:20px }
.list { width:320px }
.list ul { list-style:none; padding:0 }
.list button { width:100%; text-align:left; padding:8px; margin:4px 0; border:1px solid #ddd; background:#fff }
.viewer { flex:1 }
.meta p { margin:4px 0 }
.tabs button { margin-right:8px }
.tabs .active { font-weight:bold }
.content pre { background:#f7f7f7; padding:12px; max-height:60vh; overflow:auto }
</style>
